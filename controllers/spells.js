...
const getSingleSpell = async (req, res) => {
	try {
		...
		const spell = await db
			.collection('spells')
			.findOne({ _id: ObjectId.createFromHexString(spellId) });
...
const createSpell = async (req, res) => {
	try {
		...
		return res
			.status(500)
			.json({ message: 'Server error while creating spell' });
	}
};
...
const updateSpell = async (req, res) => {
	try {
		...
		const spellUpdates = {
			...
			...(req.body.spellType && { spellType: req.body.spellType }),
			...(req.body.use && { use: req.body.use }),
		...
		const response = await db
			.collection('spells')
			.updateOne(
				{ _id: ObjectId.createFromHexString(spellId) },
				{ $set: spellUpdates }
			);
		...
		if (response.modifiedCount > 0) {
			...
			return res
				.status(500)
				.json({ message: 'Error updating spell or spell not found' });
		}
	} catch (err) {
		console.error('Error updating spell:', err);
		return res
			.status(500)
			.json({ message: 'Server error while updating spell' });
	}
};
...
const deleteSpell = async (req, res) => {
	try {
		...
		const response = await db
			.collection('spells')
			.deleteOne({ _id: ObjectId.createFromHexString(spellId) });
		...
		if (response.deletedCount > 0) {
			...
			return res
				.status(500)
				.json({ message: 'Failed to delete spell or spell not found' });
		}
	} catch (err) {
		console.error('Error deleting spell:', err);
		return res
			.status(500)
			.json({ message: 'Server error while deleting spell' });
	}
};
